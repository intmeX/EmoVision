...
class Mixed_7a_PAD(nn.Module):
    ...
    def __init__(self):
        ...
        self.branch1 = nn.Sequential(
            BasicConv2d(896, 256, kernel_size=1, stride=1),
            BasicConv2d(256, 256, kernel_size=3, stride=2, padding=1)
        )
...
class InceptionResnetV1DLN(InceptionResnetV1):
    """Inception Resnet V1 model for DLN.
    """
    def forward(self, x):
        """Calculate embeddings for face vision or face id.
        """
        x = self.conv2d_1a(x)
        x = self.conv2d_2a(x)
        x = self.conv2d_2b(x)
...
class InceptionResnetV1DDEN(InceptionResnetV1):
    """Inception Resnet V1 model for DDEN.
    """
    def forward(self, x):
        """Calculate embeddings for face vision or face id.
        """
...
class InceptionResnetV1FPN(nn.Module):
    def __init__(self, pretrained=None, classify=False, num_classes=None, dropout_prob=0.6):
        super().__init__()
        ...
        self.conv2d_4b = BasicConv2d(192, 256, kernel_size=3, stride=2, padding=1)
        self.repeat_1 = nn.Sequential(
            Block35(scale=0.17),
            Block35(scale=0.17),
            Block35(scale=0.17),
        ...
        self.block8 = Block8(noReLU=True)
        self.fix_depth1 = nn.Conv2d(192, 224, kernel_size=1, stride=1, padding=0)
        self.fix_depth2 = nn.Conv2d(256, 224, kernel_size=1, stride=1, padding=0)
        self.fix_depth3 = nn.Conv2d(896, 448, kernel_size=1, stride=1, padding=0)
        self.fix_depth4 = nn.Conv2d(1792, 896, kernel_size=1, stride=1, padding=0)
        self.smooth1 = nn.Conv2d(224, 224, kernel_size=3, stride=1, padding=1)
        self.smooth2 = nn.Conv2d(224, 224, kernel_size=3, stride=1, padding=1)
        self.smooth3 = nn.Conv2d(448, 448, kernel_size=3, stride=1, padding=1)
        self.smooth4 = nn.Conv2d(896, 896, kernel_size=3, stride=1, padding=1)
        # self.avgpool_1a = nn.AdaptiveAvgPool2d(1)
        # self.dropout = nn.Dropout(dropout_prob)
        # self.last_linear = nn.Linear(1792, 512, bias=False)
        # self.last_bn = nn.BatchNorm1d(512, eps=0.001, momentum=0.1, affine=True)
    ...
    def forward(self, x):
        ...
        x = self.block8(x)
        f4 = x
        f4 = self.fix_depth4(f4)
        f3 = self.fix_depth3(self.reshape_like(f3, f4))
        f2 = self.fix_depth2(self.reshape_like(f2, f4))
        f1 = self.fix_depth1(self.reshape_like(f1, f4))
        f4 = self.smooth4(f4)
        f3 = self.smooth3(f3)
        f2 = self.smooth2(f2)
        f1 = self.smooth1(f1)
