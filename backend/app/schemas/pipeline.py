...
class DetectorConfig(BaseModel):
    ...
    model_size: ModelSize = Field(
        default=ModelSize.NANO, description="模型尺寸: n/s/m/l/x"
    )
    face_confidence_threshold: float = Field(
        default=0.75, ge=0.0, le=1.0, description="人脸检测置信度阈值"
    )
    person_confidence_threshold: float = Field(
        default=0.5, ge=0.0, le=1.0, description="人体检测置信度阈值"
    )
    iou_threshold: float = Field(
        default=0.45, ge=0.0, le=1.0, description="NMS IOU阈值"
    )
...
class RecognizerConfig(BaseModel):
    ...
    recognizer_type: RecognizerType = Field(
        default=RecognizerType.DDEN,
        description="识别器类型: mock/dden/emotic/caer",
    )
    model_path: Optional[str] = Field(default='./dden_checkpoint.pth', description="模型权重文件路径")
    emotion_labels: list[str] = Field(
        default=["开心", "悲伤", "愤怒", "恐惧", "惊讶", "厌恶", "中性"],
        description="情绪类别标签列表",
    )
    batch_size: int = Field(default=8, ge=1, le=64, description="批处理大小")
    use_face: bool = Field(default=True, description="使用人脸区域特征")
    use_body: bool = Field(default=True, description="使用人体区域特征")
